...
void
gldraw_first_init (GlDrawing * src)
{
...
void
setBGColor (GlDrawing * src, unsigned int color)
{
...
gboolean
gldraw_init (GstGLContext * context, GlDrawing * src,
    ...
    float bg_color_r, float bg_color_g, float bg_color_b, float bg_color_a)
{
  ...
  if (height > 0 && width > 0) {
    if (src->draw_direction == 0) {
      ...
      src->bar_step =
          round ((src->bar_len + (1.0 - src->bar_len * (channels)) / (channels +
                  2.0)) * width) / width;
      src->bars_begin =
          round (((1.0 - src->bar_step * channels) / 2.0 + (src->bar_step -
                  src->bar_len) / 2.0) * width) / width;
      src->bars_end =
          round ((src->bars_begin + src->bar_step * channels + (src->bar_step -
                  src->bar_len) / 2.0) * width) / width;
    } else {
  ...
  if (!src->shader) {
    ...
    if (len > error_message_size)
      len = error_message_size;
    memcpy (src->error_message, error->message, len);
    ret = FALSE;
  }

  src->attributes_location =
      gst_gl_shader_get_attribute_location (src->shader, "position");
  gl->BufferData (GL_ARRAY_BUFFER, 4 * sizeof (XYZW), positions,
      GL_STATIC_DRAW);
  gl->BufferData (GL_ELEMENT_ARRAY_BUFFER, 6 * sizeof (gushort), indices_quad,
      GL_STATIC_DRAW);
  gl->VertexAttribPointer (src->attributes_location, 4, GL_FLOAT, GL_FALSE,
      sizeof (GLfloat) * 4, (gpointer) (gintptr) 0);
...
gboolean
gldraw_render (GstGLContext * context, GlDrawing * src,
    loudness * audio_proceess_result)
{
  ...
  gst_gl_shader_set_uniform_4f (src->shader, "bg_color", src->bg_color.R,
      src->bg_color.G, src->bg_color.B, src->bg_color.A);
  gst_gl_shader_set_uniform_2f (src->shader, "width_height", src->width,
      src->height);
  gst_gl_shader_set_uniform_1fv (src->shader, "loud_average_arr",
      src->bar_quads_num, (float *) audio_proceess_result->loud_average_db);
  gst_gl_shader_set_uniform_1fv (src->shader, "loud_peak_arr",
      src->bar_quads_num, (float *) audio_proceess_result->loud_peak_db);
...
void
gldraw_close (GstGLContext * context, GlDrawing * src,
    loudness * audio_proceess_result)
{
